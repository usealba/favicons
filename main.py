...
def resize_favicon(input):
    ...
    resized_image.save(output_buffer, format="WEBP") # convert to webp
...
def get_favicon(url: str):    
    ...
    try:
        ...
        with open("no-favicon.webp", "rb") as f: 
    ...
    if ico_request.status_code != 200:
        ...
        resized_favicon = resize_favicon(favicon_request.content)
        
        return Response(content=resized_favicon, media_type="image/webp")
    else:
        # print(f"[DEBUG] .ico found for {url}")
        
        try:
            resized_ico = resize_favicon(ico_request.content)
        except UnidentifiedImageError:
            print("[ERROR] UnidentifiedImageError")
            with open("no-favicon.webp", "rb") as f: 
                return Response(content=f.read(), media_type="image/webp")
        
        return Response(content=resized_ico, media_type="image/webp")
